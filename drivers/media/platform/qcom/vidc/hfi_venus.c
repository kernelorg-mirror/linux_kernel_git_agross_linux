...
static int venus_hfi_core_unset_resource(struct venus_hfi_device *hdev,
					 u32 resource_id, u32 resource_size,
					 void *cookie)
{
	struct hfi_sys_release_resource_pkt pkt;
	int ret;

	if (resource_id == VIDC_RESOURCE_NONE)
		return 0;

	ret = call_hfi_pkt_op(hdev, sys_release_resource, &pkt, resource_id,
			      resource_size, cookie);
	if (ret)
		return ret;
...
static int venus_protect_cp_mem(struct venus_hfi_device *hdev)
{
	...
	ret = qcom_scm_video_mem_protect(cp_start, cp_size, cp_nonpixel_start,
					 cp_nonpixel_size);
	if (ret) {
